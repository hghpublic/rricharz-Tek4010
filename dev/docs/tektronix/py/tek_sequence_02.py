...
def clear_screen():
    """
    FIXED: Sends the mandatory ESC + FF sequence required by the 
    Tektronix protocol to reliably trigger full screen erasure.
    """
    sys.stdout.buffer.write(ESC + FF)
    sys.stdout.buffer.flush()
    # Physical storage tubes require a brief moment to flash/erase the screen 
    time.sleep(0.3) 
